...
| Evolution | Minimum wave/VM budget covers spawned preparation and apply admission. Framed result reception polls; every started child is stopped/reaped. Source-lock/revalidation checkpoints precede journal admission. | Startup, serialization and reaping can exceed the budget. Entered journal recovery/commit finishes. Source may change before timeout; activation is next-process only. Interrupted candidate caches can remain. |
| HTTP serving | Serving context is checked by the accept loop and inherited by handlers. Queue/input/execution/output share each request's deadline. Normal shutdown drains admitted handlers; context cancellation aborts sockets and joins workers. | Response loss does not undo an application commit. Noncooperative handlers or host operations can delay final joining. Detailed admission and drain boundaries remain in the parallel amendment. |
