# Explicit plaintext storage migration

This amendment implements the separately frozen design. The baseline under
...
Use `gopyt-store migrate --root ROOT --backup ABSOLUTE_PATH --expected-digest SHA256`.
...
Qualification covers the real host API and CLI, independent decryption plus full SQLite row
...
crashes around both backup and active durability/publication barriers. Contention and cancellation use the existing store deadline rules. Source identities
and unsuccessful trials are retained with qualification evidence. Process exits
are not power-loss testing; no production recovery-time or security certification is
claimed by finite functional tests.


## Recovery files and configuration

The backup parent directory must already exist, have private permissions (no group
or other access), and be owned by the effective user or root. Recovery files are
private single-link regular files. The runtime reserves
`.gopyt-migration-<SHA256 of backup basename>` as staging and the same name with
`.lock` as a persistent coordination lock. Do not modify these names while any
migration process is active. The lock uses the store's existing deadline/context
checks. Different stores claiming the same backup path serialize through that lock.

Publication uses an exclusive staging file and a no-overwrite hard link, followed
by staging unlink and directory fsync. A crash between link and unlink leaves two
links to the same encrypted inode; retry recognizes only the exact reserved staging
link and completes cleanup. A partial unpublished staging file is removed under
the recovery lock only after the active source has been validated. Existing recovery
copies must authenticate using the selected active key, not merely a retained reader
key. Keep the same key selection, store identity, digest and backup path for retry.

The host API is `Store.migrate(backup=..., expected_digest=...)`. CLI success emits
JSON with `operation`, `status`, `source_digest` and `ciphertext_digest`; errors exit
2. No key or database values are printed. Synchronous SQLite integrity checks,
cryptographic calls and individual filesystem operations retain their documented
non-interruptible host-call limits. Cancellation before active publication leaves
the old active file and may leave a durable encrypted recovery copy; retry validates
that copy before use. Once active replacement is admitted, its durability barrier
