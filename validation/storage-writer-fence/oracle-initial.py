...
with tempfile.TemporaryDirectory() as temporary:
 ...
 with patch.dict(os.environ,env):
  ...
  with patch.dict(os.environ,{'GOPYT_STORE_KEYRING_FILE':rings['retired']}):
   child=subprocess.run([sys.executable,'-c','from gopyt.storage import Store; import sys; assert Store(sys.argv[1]).get("balance")=="10"',str(root)],capture_output=True,text=True);assert child.returncode==0,child.stderr
...
'''
  child=subprocess.run([sys.executable,'-c',code,str(wheel),str(anchor/'record.json')],capture_output=True,text=True);assert child.returncode==0,child.stderr;assert child.stdout.strip()=='v1-reader-rejected-v2'
