...
def validate(name, facts, config):
    if name == 'slow_clients':
        ...
        require(facts['kind'] in ('local', 'process'), 'lock kind')
        require(facts['kind'] != 'process' or facts['process_blocked'], 'process lock not reached')
        ...
        require(facts['response'] == [200, 'true'] and facts['closed'], 'drain acknowledgment')
