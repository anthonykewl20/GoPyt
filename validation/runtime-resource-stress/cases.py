...
def cancellation_storm(config, iteration, mark):
    ...
    try:
        ...
        return dict(outcomes=sorted(outcomes), entered=entered, reserved=reserved,
                    ...
                    handoffs=cancelled_handoffs, recovered=recovery_ok,
...
def lock_contention(config, iteration, mark):
    ...
    process_blocked = threading.Event()
    ...
    try:
        with patch.object(vm.db, '_lock_remaining', remaining), patch('gopyt.storage.fcntl.flock', flock):
            try:
                ...
                if kind == 'process' and not process_blocked.wait(10):
                    raise AssertionError('process lock contention watchdog')
                peak = mark()
                started = time.perf_counter_ns()
                expire.set()
        ...
        return dict(kind=kind, process_blocked=process_blocked.is_set(),
                    blocked=len(seen), outcomes=sorted(outcomes),
...
task commit(key: str) -> bool
    effects { database.write }
{
    result = store.db.put(key, "committed")
    return match result {
        unit -> true
        DbError { message } -> false
    }
}
